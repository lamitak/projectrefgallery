...
export async function onRequest(context) {
  ...
  try {
    ...
    if (path === "/skus/bulk" && method === "POST") {
      const body = await request.json();
      const { skus: skuList, categories: newCats } = body;
      // Add new categories
      ...
      for (const s of skuList) {
        try {
          const result = await db.prepare("INSERT INTO skus (code, name, category) VALUES (?, ?, ?)").bind(s.code.toUpperCase(), s.name, s.category || "Uncategorized").run();
          added.push({ id: result.meta.last_row_id, ...s });
        } catch (e) {
          // Skip duplicates
        }
      }
      return json({ added: added.length, skus: added }, 201);
    }
    ...
    if (path === "/projects" && method === "POST") {
      const body = await request.json();
      const { name, date, designer, photographer, description, internalOnly, skuIds } = body;
      if (!name) return err("name required");
      const result = await db.prepare(
        "INSERT INTO projects (name, date, designer, photographer, description, internal_only) VALUES (?, ?, ?, ?, ?, ?)"
      ).bind(name, date || "", designer || "", photographer || "", description || "", internalOnly ? 1 : 0).run();
    ...
    if (path.startsWith("/projects/") && !path.includes("/images") && method === "PUT") {
      const id = parseInt(path.split("/")[2]);
      const body = await request.json();
      const { name, date, designer, photographer, description, internalOnly, skuIds } = body;
      await db.prepare(
        "UPDATE projects SET name = ?, date = ?, designer = ?, photographer = ?, description = ?, internal_only = ? WHERE id = ?"
      ).bind(name, date || "", designer || "", photographer || "", description || "", internalOnly ? 1 : 0, id).run();
      // Re-sync SKU tags
    ...
    if (path.startsWith("/projects/") && !path.includes("/images") && method === "DELETE") {
      const id = parseInt(path.split("/")[2]);
